...
(function () {
  ...
  /* ---------------------------------------------------------------
     Значок вкладки — не здесь
     ---------------------------------------------------------------
     Он задан в настройках сайта на портале (Дополнительно → Favicon) и
     приходит в <head> готовой страницы: пятью размерами сразу и без JS.
     Раньше ссылки вставлял этот файл — тогда поле в настройках было пустым,
     потому что REST его не отдаёт, а блок «свой код в <head>» не годится:
     проверка безопасности портала ломает там тег link, сохранённое
     возвращается как «<li nk …>». Настройку заполнили через интерфейс, и
     вставку убрали, чтобы две разные не спорили за <head>. */
